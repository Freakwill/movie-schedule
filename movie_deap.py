...
class Manager:
    ...
    # def random(self):
    #     halls = self.halls
    #     tm = np.random.randint(0, 11, size=(len(halls), 6))
    #     for h in self.halls:
    ...
    def check(self):
        ...
        print('最小时间间隔: %.4f+%.4f;\n热度与场次比近似程度: %.4f;\n总热度(黄金时段): %.4f(%.4f);\n放映满电影影厅个数: %d'%(d1, d2, self.check_rate(), self.total_hot(), self.ghot(), self.check_time()))
...
def mycx(ind1, ind2):
    s1 = set(h[0] for h in ind1)
    s2 = set(h[0] for h in ind2)
    if random.random() > 1/(len(s1.symmetric_difference(s2))+1):
...
if __name__ == "__main__":
    ...
    # ga = vcga.RandomParallelGA(algorithms.eaSimple, epochs=15, send_best=15)                
    # ga(pops, toolbox, cxpb=0.6, mutpb=0.173, ngen=15, verbose=False)
    # ind = ga.selBest(pops)
    manager.schedule(ind)
    manager.print_fitness()
    aga = vcga.AdaptiveGA(algorithms.eaSimple, epochs=13)
    pga = vcga.RandomParallelGA(aga, epochs=7, send_best=10) 
    pga(pops, toolbox, cxpb=0.7, mutpb=0.32, ngen=2, verbose=False)
